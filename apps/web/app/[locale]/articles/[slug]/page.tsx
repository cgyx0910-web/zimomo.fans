...
import {
  ...
  listPublishedLocalesForArticleSlug,
...
import { buildArticleLanguageAlternates } from "@/lib/seo/article-alternates";
...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  try {
    article = await getPublishedArticleBySlug(slug, locale);
  } catch {
    return { title: "资讯" };
  }
  ...
  const canonical = article.canonicalUrl?.trim()
    ...
    : absoluteArticlePath(article.slug, locale);
  ...
  let languages: NonNullable<Metadata["alternates"]>["languages"] | undefined;
  try {
    const locs = (await listPublishedLocalesForArticleSlug(slug)).filter(
      (l): l is AppLocale => isAppLocale(l)
    );
    if (locs.length > 1) {
      languages = buildArticleLanguageAlternates(slug, locs);
    }
  } catch {
    languages = undefined;
  }
...
export default async function ArticleDetailPage(props: PageProps) {
  ...
  const canonical = article.canonicalUrl?.trim() ?? absoluteArticlePath(slug, locale);

  const faqItems = toEditorialFaqItems(article.editorialFaq);
  const faqLang = locale === "en" ? "en" : "zh-CN";
  const faqJsonLd =
    faqItems.length > 0 ? buildFaqPageJsonLd(canonical, faqItems, faqLang) : null;
    ...
    <article className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-14">
      <header className="space-y-3 border-b border-neutral-200 pb-6 dark:border-neutral-800">
        <p className="text-xs text-neutral-500">
          <Link className="underline" href={localePath(locale, "/articles")}>
            资讯
          </Link>
        ...
        <section className="rounded-lg border border-neutral-200 bg-neutral-50 p-4 text-sm dark:border-neutral-800 dark:bg-neutral-900/40">
          ...
          <p className="mt-2 text-neutral-700 dark:text-neutral-300">
            ...
            <Link
              ...
              href={localePath(locale, `/clusters/${relatedHub.slug}`)}
            >
