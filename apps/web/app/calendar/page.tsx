...
export default async function CalendarPage(props: { searchParams: Promise<Search> }) {
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-4 py-14">
      <header className="space-y-3 border-b border-neutral-200 pb-6 dark:border-neutral-800">
        ...
        <p className="text-sm font-medium">
          <a className="text-emerald-700 underline hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300" href="/calendar/feed.ics">
            订阅 ICS
          </a>
      ...
      <nav className="flex flex-wrap items-center justify-between gap-3 text-sm">
        <div className="flex flex-wrap gap-2">
          <Link
            ...
            href={`/calendar?month=${ym(prev.y, prev.m)}`}
          >
            上一月
          </Link>
          <Link
            className="rounded-md border border-neutral-300 px-3 py-1.5 hover:bg-neutral-50 dark:border-neutral-700 dark:hover:bg-neutral-900"
            href={`/calendar?month=${ym(next.y, next.m)}`}
          >
            下一月
          </Link>
          <Link
      ...
      <section>
        ...
        {rows.length === 0 ? (
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            本月暂无已发布活动。{" "}
            <Link className="underline" href={`/calendar?month=${ym(next.y, next.m)}`}>
              查看下一月
            </Link>
            或通过 ICS 持续关注。
          </p>
