...
const PORT = Number(
  process.env.B2_VERIFY_PORT ?? 3800 + Math.floor(Math.random() * 200)
);
...
async function verifyWebhookAndSitemap(secret, allowAuthorizedRun) {
  ...
  if (allowAuthorizedRun) {
    ...
    assert(
      typeof summary.totalStoredCount === "number",
      "B2 ingest summary 缺少 totalStoredCount"
    );
  } else {
    console.log("[B2-verify] skip authorized ingest run (DATABASE_URL not set)");
  }

  const sitemapRes = await fetch(`${BASE_URL}/sitemap.xml`);
  const sitemap = await sitemapRes.text();
  assert(!sitemap.includes("/admin/raw-documents"), "sitemap 不应包含后台 raw 页面");
  assert(!sitemap.includes("/api/ingest/rss"), "sitemap 不应包含 ingest API");
}
...
async function main() {
  const env = {
    ...
    INGEST_WEBHOOK_SECRET: "b2-test-secret",
    RAW_BLOB_LOCAL_DIR: ".data/raw-blobs-test",
  };

  console.log("[B2-verify] build web...");
  run("pnpm --filter web build", env);

  console.log("[B2-verify] check webhook and sitemap constraints...");
  ...
  console.log("[B2-verify] phase B2 checks passed");
}

main().catch((error) => {
  console.error("[B2-verify] failed:", error.message);
  process.exitCode = 1;
});
